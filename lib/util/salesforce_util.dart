...
class SalesforceUtil{
  ...
  static String clientSecret = dotenv.env['clientSecret'] ?? '';
  static String userName = dotenv.env['userName'] ?? '';
  static String pwdWithToken = dotenv.env['pwdWithToken'] ?? '';
  static String tokenEndpoint = dotenv.env['tokenEndpoint'] ?? '';
  static String tokenGrantType = dotenv.env['tokenGrantType'] ?? '';
  static String compositeUrlForInsert = dotenv.env['compositeUrlForInsert'] ?? '';
  static String compositeUrlForUpdate = dotenv.env['compositeUrlForUpdate'] ?? '';
  ...
  static Future<Map<String, dynamic>> dmlToSalesforce({String opType = '', String objAPIName = '', List<Map<String, dynamic>> fieldNameValuePairs = const [], List<String> recordIds = const [], bool hardDelete = false, int batchSize = 200}) async{
    ...
    if(opType == 'delete'){
      while(recordIds.isNotEmpty){
        ...
        if(debug) log.d('resp in delete : $resp');
      ...
      while(fieldNameValuePairs.isNotEmpty){
        ...
        if(debug) log.d('resp in insert update : $resp');
    ...
    if(debug) log.d('Final value of dmlToSalesforceResponse=>' + dmlToSalesforceResponse.toString());
  ...
  static Future<Map<String, dynamic>> queryFromSalesforce({ required String objAPIName, List<String> fieldList = const [], String whereClause = '', String orderByClause = '', int? count}) async {
    ...
    if(debug) log.d('Response is for records : ${resp.toString()}');
    bool done = (resp.containsKey('done') && resp['done']) ? true : false;
    if(done){
      if(resp.containsKey('data')){
        queryFromSalesforceResponse['data'] = resp;
        if(debug) log.d('I am here ${queryFromSalesforceResponse['data'].toString()}');
      }
      ...
      if(debug) log.d('queryFromSalesforce nextRecordsUrl =>$nextRecordsUrl');
      if(debug) log.d('queryFromSalesforce url =>$instanceUrl$nextRecordsUrl');
      ...
      if(debug) log.d('Rest query response : ${body.toString()}');
      bool done = (resp.containsKey('done') && resp['done']) ? true : false;
      if(done){
    ...
    if(debug) log.d('Result from queryFromSalesforceResponse $queryFromSalesforceResponse');
  ...
  static Future<Map<String, dynamic>> _login() async{
    ...
    try{
      ...
      if (resp.statusCode == 200) {
        ...
        // Log an error
        if(debug) log.d('Response code other than 200 detected : ${resp.body}');
        loginResponse['error'] = body.toString();
      }
      // responseMap.add('data') = response.body;
    }
    catch(error){
      if(debug) log.d('Error occurred while logging into Salesforce. Error is : $error');
  ...
  static Future<Map<String, dynamic>> _dmlToSalesforce(String opType, String objAPIName, List<Map<String, dynamic>> fieldNameValuePairs, {int batchCount = 0}) async{
    ...
    try{
      if(opType == 'insert'){
        ...
        if(debug) log.d('body for insert : $body');
      }
      else{
        body = await _updateToSalesforce(objAPIName, fieldNameValuePairs, batchCount);
        if(debug) log.d('body for update : $body');
      }

      if(debug) log.d('I am here line 236');
      if(debug) log.d('body[] : ${body.toString()}');
      // Collate the response for all batches
      if(body.containsKey('data') && body['data'].isNotEmpty){
        if(debug) log.d('I am here line 227 and data is ${body['data']}');
      ...
      if(debug) log.d('I am here line 253');
    }
    catch(error){
      if(debug) log.d('body for error scenario : $body');
      List<dynamic> catchBlockErrors = [];
      catchBlockErrors.add(error.toString());
      dmlResponse['errors'] = catchBlockErrors;
    }
    if(debug) log.d('DML response for $batchCount : $dmlResponse');
    return dmlResponse;
  }
  ...
  static Future<Map<String, dynamic>> _insertToSalesforce(String objAPIName, List<Map<String, dynamic>> fieldNameValuePairs, int batchCount) async{
    ...
    try{
      ...
      if(statusCode == 201 && !body['hasErrors']){
        if(debug) log.d('Inside 201 $body');
        insertResponse['data'] = body['results'];
      }
      else{ // non 201 code is returned
        if(debug) log.d('Response code other than 200/201 detected $statusCode');
        if(debug) log.d('outside 201 $body');
        if(body['hasErrors']){
    ...
    if(debug) log.d('Final insertResponse output : $insertResponse');
  ...
  static Future<Map<String, dynamic>> _updateToSalesforce(String objAPIName, List<Map<String, dynamic>> fieldNameValuePairs, int batchCount) async{
    ...
    try{
      ...
      if(debug) log.d('Inside _updateToSalesforce StatusCode: ${resp.statusCode} || body: $body || updateResponse: $updateResponse');
    ...
    if(debug) log.d('Response from _updateToSalesforce $updateResponse');
  ...
  static Future<Map<String, dynamic>> _deleteFromSalesforce(String objAPIName, List<String> recordIds, int batchCount, bool hardDelete) async{
    ...
    try{
      ...
      final List<dynamic> body = json.decode(resp.body);
      if(debug) log.d('StatusCode: ${resp.statusCode} || body: $body || deleteResponse : $deleteResponse');
      deleteResponse = processDMLResponse2(statusCode : resp.statusCode, inputBody : body, inputResponse: deleteResponse);
      if(debug) log.d('After processResponse $deleteResponse');
    }
    catch(error){
      deleteResponse['error'] = error.toString();
    }
    if(debug) log.d('Response from _deleteResponse $deleteResponse');
  ...
  static Future<Map<String, dynamic>> _queryFromSalesforce(String objAPIName, List<String> fieldList, String whereClause, String orderByClause, int? count) async {
    ...
    if(debug) log.d('instanceUrl inside _queryFromSalesforce $instanceUrl');
    ...
    try{
      ...
      final Map<String, dynamic> body = json.decode(resp.body);
      if(debug) log.d('_queryFromSalesforce response.statusCode ${resp.statusCode}');
      if(debug) log.d('_queryFromSalesforce body : ${body.toString()}');
      if(debug) log.d('_queryFromSalesforce body : $body');
      if (resp.statusCode == 200) {
        if(debug) log.d('_queryFromSalesforce resp[done] : ${body['done']}');
        if(debug) log.d('_queryFromSalesforce resp[totalSize] : ${body['totalSize']}');
        if(debug) log.d('_queryFromSalesforce resp[nextRecordsUrl] : ${body['nextRecordsUrl']}');
      ...
      if(debug) log.d('Error occurred while querying data from Salesforce. Error is : $error');
      queryFromSlesforceResponse['error'] = error.toString();
    }
    if(debug) log.d('queryFromSlesforceResponse=> $queryFromSlesforceResponse');
  ...
  static String generateEndpointUrl({required String opType, String objAPIName = '', List<String> recordIds = const [], int batchCount = 0, bool hardDelete = false}){
    ...
    if(debug) log.d('Generated endpoint : $endpointUrl');
    return endpointUrl;
  }
  ...
  static String generateQueryEndpointUrl(String objAPIName, List<String> fieldList, String whereClauseString, String orderByClauseString, int? count){
    ...
    // if(debug) log.d('Encoded Query : $query');
    
    final String endpointUrl = '$instanceUrl$queryUrl$query';
    if(debug) log.d('endpointUrl inside generateQueryEndpointUrl: $endpointUrl');
  ...
  static dynamic processDMLResponse2({required int statusCode, required List<dynamic> inputBody, required Map<String, dynamic> inputResponse}){
    for(dynamic rec in inputBody){
      if(statusCode == 200){
        ...
        if(debug) log.d('Response code other than 200 detected : $statusCode');
